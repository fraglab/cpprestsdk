...
#include "cpprest/details/read_until.h"
...
using namespace web::details::asio::ip;
...
namespace
{
    struct iequal_to
    {
        ...
        {
            return lib::ilexicographical_compare(left, right);
        }
    };
    ...
    private:
        ...
        bool m_is_https;
        const std::function<void(lib::asio::ssl::context&)>& m_ssl_context_callback;
    ...
    private:
        void on_accept(std::unique_ptr<tcp::socket> socket, const lib::asio::error_code& ec);
...
private:
    ...
    typedef void (asio_server_connection::*ResponseFuncPtr) (const http_response &response, const asio::error_code& ec);

    std::unique_ptr<tcp::socket> m_socket;
    lib::asio::streambuf m_request_buf;
    lib::asio::streambuf m_response_buf;
    http_linux_server* m_p_server;
    ...
    using ssl_stream = lib::asio::ssl::stream<tcp::socket&>;

    std::unique_ptr<lib::asio::ssl::context> m_ssl_context;
    std::unique_ptr<ssl_stream> m_ssl_stream;

    asio_server_connection(std::unique_ptr<tcp::socket> socket, http_linux_server* server, hostport_listener* parent)
...
public:
    ...
    static refcount_ptr create(std::unique_ptr<tcp::socket> socket, http_linux_server* server, hostport_listener* parent)
    {
    ...
    will_erase_from_parent_t start_connection(bool is_https, const std::function<void(lib::asio::ssl::context&)>& ssl_context_callback)
    {
        auto unique_reference = this->get_reference();

        if (is_https)
        {
            m_ssl_context = make_unique<lib::asio::ssl::context>(lib::asio::ssl::context::sslv23);
            ...
            m_ssl_stream->async_handshake(lib::asio::ssl::stream_base::server, [this](const lib::asio::error_code&)
            {
...
private:
    ...
    will_deref_and_erase_t start_request_response();
    will_deref_and_erase_t handle_http_line(const lib::asio::error_code& ec);
    will_deref_and_erase_t handle_headers();
    will_deref_t handle_body(const lib::asio::error_code& ec);
    will_deref_t handle_chunked_header(const lib::asio::error_code& ec);
    will_deref_t handle_chunked_body(const lib::asio::error_code& ec, int toWrite);
    ...
    will_deref_and_erase_t handle_headers_written(const http_response &response, const lib::asio::error_code& ec);
    will_deref_and_erase_t handle_write_large_response(const http_response &response, const lib::asio::error_code& ec);
    will_deref_and_erase_t handle_write_chunked_response(const http_response &response, const lib::asio::error_code& ec);
    will_deref_and_erase_t handle_response_written(const http_response &response, const lib::asio::error_code& ec);
...
namespace asio
{
template <> struct is_match_condition<crlfcrlf_nonascii_searcher_t> : public std::true_type {};
}
...
void hostport_listener::start()
{
    ...
    m_acceptor->async_accept(*socket, [this, socket](const lib::asio::error_code& ec)
    {
        std::unique_ptr<ip::tcp::socket> usocket(socket);
...
will_deref_and_erase_t asio_server_connection::start_request_response()
{
    ...
    if (m_ssl_stream)
    {
        lib::asio::async_read_until(*m_ssl_stream, m_request_buf, CRLFCRLF, [this](const lib::asio::error_code& ec, std::size_t)
        {
    ...
    {
        lib::asio::async_read_until(*m_socket, m_request_buf, crlfcrlf_nonascii_searcher, [this](const lib::asio::error_code& ec, std::size_t)
        {
            (will_deref_and_erase_t)this->handle_http_line(ec);
        });
    }
...
void hostport_listener::on_accept(std::unique_ptr<ip::tcp::socket> socket, const lib::asio::error_code& ec)
{
    // Listener closed
    ...
    if (!ec)
    {
        ...
        catch (lib::asio::system_error&)
        {
            // asio ssl apis throw asio::system_error.
    ...
    if (m_acceptor)
    {
        ...
        m_acceptor->async_accept(*newSocket, [this, newSocket](const lib::asio::error_code& ec)
        {
            std::unique_ptr<ip::tcp::socket> usocket(newSocket);
            this->on_accept(std::move(usocket), ec);
        });
...
will_deref_and_erase_t asio_server_connection::handle_http_line(const lib::asio::error_code& ec)
{
    ...
    if (ec)
    {
        ...
        if (
            ec == lib::asio::error::eof ||                // peer has performed an orderly shutdown
            ec == lib::asio::error::operation_aborted ||  // this can be removed. ECONNABORTED happens only for accept()
            ec == lib::asio::error::connection_reset ||   // connection reset by peer
            ec == lib::asio::error::timed_out             // connection timed out
        )
        {
    ...
    else
    {
        ...
        if (lib::iequals(http_verb, methods::GET))          http_verb = methods::GET;
        else if (lib::iequals(http_verb, methods::POST))    http_verb = methods::POST;
        else if (lib::iequals(http_verb, methods::PUT))     http_verb = methods::PUT;
        else if (lib::iequals(http_verb, methods::DEL))     http_verb = methods::DEL;
        else if (lib::iequals(http_verb, methods::HEAD))    http_verb = methods::HEAD;
        else if (lib::iequals(http_verb, methods::TRCE))    http_verb = methods::TRCE;
        else if (lib::iequals(http_verb, methods::CONNECT)) http_verb = methods::CONNECT;
        else if (lib::iequals(http_verb, methods::OPTIONS)) http_verb = methods::OPTIONS;
...
will_deref_and_erase_t asio_server_connection::handle_headers()
{
    ...
    while (std::getline(request_stream, header) && header != "\r")
    {
        ...
        if (colon != std::string::npos && colon != 0)
        {
            ...
            if (lib::iequals(name, header_names::content_length))
            {
    ...
    {
        m_close = lib::iequals(name, U("close"));
    }

    if (currentRequest.headers().match(header_names::transfer_encoding, name))
    {
        m_chunked = lib::ifind_first(name, U("chunked"));
    }
    ...
    else // need to read the sent data
    {
        m_read = 0;
        ++m_refs;
        async_read_until_buffersize(std::min(ChunkSize, m_read_size), [this](const lib::asio::error_code& ec, size_t)
        {
...
will_deref_t asio_server_connection::handle_chunked_header(const lib::asio::error_code& ec)
{
    ...
    else
    {
        ...
        {
            async_read_until_buffersize(len + 2, [this, len](const lib::asio::error_code& ec, size_t)
            {
...
will_deref_t asio_server_connection::handle_chunked_body(const lib::asio::error_code& ec, int toWrite)
{
    auto requestImpl = get_request()._get_impl();
    if (ec)
    {
...
will_deref_t asio_server_connection::handle_body(const lib::asio::error_code& ec)
{
    auto requestImpl = get_request()._get_impl();
    // read body
    if (ec)
    ...
    {
        auto writebuf = requestImpl->outstream().streambuf();
        writebuf.putn_nocopy(lib::asio::buffer_cast<const uint8_t*>(m_request_buf.data()), std::min(m_request_buf.size(), m_read_size - m_read)).then([this](pplx::task<size_t> writtenSizeTask) -> will_deref_t
        {
            ...
            async_read_until_buffersize(std::min(ChunkSize, m_read_size - m_read), [this](const lib::asio::error_code& ec, size_t)
            {
                (will_deref_t) this->handle_body(ec);
            });
            return will_deref_t{};
...
will_deref_and_erase_t asio_server_connection::async_write(WriteFunc response_func_ptr, const http_response &response)
{
    if (m_ssl_stream)
    {
        lib::asio::async_write(*m_ssl_stream, m_response_buf, [=] (const lib::asio::error_code& ec, std::size_t)
        {
    ...
    {
        lib::asio::async_write(*m_socket, m_response_buf, [=] (const asio::error_code& ec, std::size_t)
        {
            (this->*response_func_ptr)(response, ec);
        });
    }
...
will_deref_t asio_server_connection::async_handle_chunked_header()
{
    if (m_ssl_stream)
    {
        lib::asio::async_read_until(*m_ssl_stream, m_request_buf, CRLF, [this](const lib::asio::error_code& ec, size_t)
        {
    ...
    {
        lib::asio::async_read_until(*m_socket, m_request_buf, CRLF, [this](const lib::asio::error_code& ec, size_t)
        {
            (will_deref_t)this->handle_chunked_header(ec);
        });
    }
...
void asio_server_connection::async_read_until_buffersize(size_t size, const ReadHandler &handler)
{
    ...
    if (m_ssl_stream)
    {
        lib::asio::async_read(*m_ssl_stream, m_request_buf, condition, handler);
    }
    else
    {
        lib::asio::async_read(*m_socket, m_request_buf, condition, handler);
    }
}
...
void asio_server_connection::serialize_headers(http_response response)
{
    ...
    for(const auto & header : response.headers())
    {
        ...
        if (lib::iequals(header.first, U("connection")))
        {
            if (lib::iequals(header.second, U("close")))
            {
                m_close = true;
            }
        }
...
will_deref_and_erase_t asio_server_connection::handle_write_chunked_response(const http_response &response, const lib::asio::error_code& ec)
{
    if (ec)
    {
...
will_deref_and_erase_t asio_server_connection::handle_headers_written(const http_response &response, const lib::asio::error_code& ec)
{
    if (ec)
    {
